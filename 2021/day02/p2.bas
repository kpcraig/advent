...
64 PRINT C$
65 PRINT A$
66 PRINT V:PRINT H: PRINT A
70 IF C$ = "f" THEN H = H + VAL(A$): V = V + A * VAL(A$)
71 IF C$ = "d" THEN A = A + VAL(A$)
72 IF C$ = "u" THEN A = A - VAL(A$)
100 GET#1,A$: REM Consume carriage return
110 GOTO 20
200 CLOSE 1
210 PRINT "VERT:": PRINT V
220 PRINT "HORZ:": PRINT H
230 PRINT V * H
